...
const sharp = require('sharp'); // this is for resize image.
...
//So when there's one only, then it's upload dot single, let's say image, when there is multiple
//  with the same name, then it's upload dot array and when there's basically a mix of them,
// then it is upload dot fields().
...
exports.resizeToursImages = catchAsync(async (req, res, next) => {
  ...
  req.body.imageCover = imageCoverFilename; // update the req, use in DB insert/update
